...
import click
from platon_utility import write_csv, read_csv, transaction_str_to_int, \
    sign_one_transaction_by_prikey, get_dir_by_name, get_private_key_from_wallet_file, \
    signed_transaction_file_dir, get_password_file


@click.command(help='签名普通锁仓交易文件')
@click.option('-f', '--filepath', 'filePath', required=True, help='锁仓交易待签名文件路径.')
@click.option('-k', '--keystore', help='签名锁仓交易钱包文件路径.')
def batch_signed_restrict_tx(filePath, keystore):
    try:
        ...
        for one_transaction in transaction_list:
            ...
            i = i + 1
            print("完成第: %d 笔%s交易签名=================" % (i, tx_type))

        # 生成 csv 文件
        signed_file_csv_name = "signed_restrict_transaction.csv"
        ...
        print('generate signed restrict transaction file failure!!!')
        sys.exit(1)

    else:
        print('{}{} {}'.format('SUCCESS\n', "generate signed restrict transaction file:", signed_file_path))


if __name__ == "__main__":
    batch_signed_restrict_tx()
