...
from platon_utility import write_csv, read_csv, transaction_str_to_int, \
    sign_one_transaction_by_prikey, get_private_key_from_wallet_file, get_dir_by_name, \
    signed_transaction_file_dir, get_password_file


@click.command(help="签名普通转账")
@click.option('-f', '--filepath', 'filePath', required=True, help='待签名文件路径.')
@click.option('-k', '--keystore', required=True, help='签名交易钱包文件所在路径.')
def batch_signed_transfer_tx(filePath, keystore):
    try:
        ...
        # 生成 csv 文件
        signed_file_csv_name = "signed_transfer_transaction.csv"
        ...
        print('generate signed transfer transaction file failure!!!')
        sys.exit(1)

    else:
        print('SUCCESS\nGenerate signed transfer transaction file: %s' %
              signed_file_path)


if __name__ == "__main__":
    batch_signed_transfer_tx()
